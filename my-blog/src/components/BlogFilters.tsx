...
interface BlogFiltersProps {
  allTags: string[]
  selectedTags: string[]  // 支持多选
  searchTerm: string
  onTagSelect: (tags: string[]) => void  // 传递标签数组
          ...
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-2xl p-6 border-2 border-blue-200 dark:border-blue-700">
            <div className="flex flex-wrap justify-center items-center gap-4">
              ...
              {selectedTags.map((tag) => (
                <span key={tag} className="inline-flex items-center px-4 py-2 bg-blue-500 text-white rounded-xl shadow-md hover:shadow-lg transition-all">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {popularTags.map((tag) => (
                ...
                >
                  ...
                  {tag}
                  {selectedTags.includes(tag) && (
                    <span className="ml-2 bg-blue-400 text-blue-100 rounded-full w-5 h-5 flex items-center justify-center text-xs">
                      ✓
                    </span>
                  )}
                </button>
